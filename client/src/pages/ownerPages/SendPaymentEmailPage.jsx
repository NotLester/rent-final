				...
				<form onSubmit={handleSendConfirmation}>
					...
					<div className="flex mt-2 gap-2 items-start">
						...
						<div className="text-sm mt-1">
							...
							<br />
							<p>
								I hope this email finds you well. This is a
								...
								is due. As per our rental agreement, rent is to
								be paid within 7 days after{" "}
								{dateFormatter(
									rentDetail?.currentRentDate.from
								)}
								. The total rent amount is INR{" "}
								{format(
									calculateTotalRent(
										rentDetail?.paymentPlan,
										rentDetail?.realEstate.price
								...
								for INR {format(rentDetail?.realEstate?.price)}{" "}
								per month.
							</p>
							<br />
							<p>
								Please note that late payment fees may apply if
								the rent is not paid by the due date. If you
								have any questions or concerns, please do not
								hesitate to contact me. Thank you for your
								cooperation. Kindly contact me at{" "}
								{rentDetail?.owner?.email} or +977{" "}
							...
							<p>Best regards,</p>
							<p>
								{rentDetail?.owner?.firstName}{" "}
								{rentDetail?.owner?.lastName}
							</p>
						</div>
					</div>
